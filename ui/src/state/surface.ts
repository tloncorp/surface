...
export const useSurfaceState = create<SurfaceState>()(
      ...
      (set, get) => ({
        ...
        removeSurface: id => {
          ...
          set(draft => {
            ...
            const newActiveSurface =
              activeSurface === id
                ? newSurfaces[index - 1]?.id ?? null
                : activeSurface;
            return {
        ...
        combineSurfaces: (draggedId, dropTargetId) => {
          ...
          if (draggedIndex === -1) {
            console.warn(
              'attempted to combine non-existent surface',
              draggedId
            );
            return;
          }
...
export function useEditWidget(
  ...
  const pane = usePaneFromWidget(surfaceId, widgetId);
  const surface = useSurface(surfaceId);
  const { widgets } = pane as WidgetPane;
  ...
  const handleWidgetEdited = useCallback(
    (updatedWidget: Widget) => {
      ...
      if (!surface) {
        console.warn(
          'attempted to add widget to non-existent surface',
          surfaceId
        );
        return;
      }
